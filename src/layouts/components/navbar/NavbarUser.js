...
const UserDropdown = (props) => {
  ...
  const onClickLogout = useCallback(
    async (e) => {
      ...
      const user_type = UserPreference.getInstance().getUserType();
      let path = '/client_panel';
  ...
  return (
    <DropdownMenu right>
      ...
      <DropdownItem tag='a' href='/fivestar_panel/login' onClick={onClickLogout}>
  ...
  render() {
        ...
        <div className='cart-item' key={item.id}>
          <Media className='p-0' onClick={() => history.push('/ecommerce/product-detail')}>
            ...
            <Media className='overflow-hidden pr-1 py-1 pl-0' body>
              <span className='item-title text-truncate text-bold-500 d-block mb-50'>{item.name}</span>
    ...
    return (
      <ul className='nav navbar-nav navbar-nav-user float-right'>
        {/*
    Remove Language and Cart from header
